...
class TestUser(unittest.TestCase):
    ...
    def test_short_url(self): # comment out this line "request.urlopen(url)" in ahorten_url.py file in models dir, the functions that validates the url
        data = {
            "original_url": "https://www.mygreatlearning.com/blog/top-django-projects/#hospital-management-system"
        }
        res = self.client.post("/api/v1/user/short_url/create", json=data, headers=self.headers)
    ...
    def test_create_qr_code(self):
        ...
        res = self.client.post("/api/v1/qr_code/qrcode", json=data, headers=self.headers)
